<?php
    if(isset($_GET['orderId'])&&($_GET['orderId'])){
        ...
        $sqluser = mysqli_query($con,"SELECT * FROM nguoidung ORDER BY id_nguoidung='$iduser'");
		...
		$sql_hoadon = mysqli_query($con,"INSERT INTO hoadon(id_hoadon,id_nguoidung,sdt,diachi,tonghd,ngayhd,ghichu,thanhtoan,tinhtrang,trangthai) values ('$id_hd','$iduser','$phone','$address','$tong','$date','$note','Thanh toán qua Momo','$tinhtrang','1')");
		$sql_lay_giohang = mysqli_query($con,"SELECT * FROM giohang ORDER BY id_giohang DESC");
		while($row_thanhtoan = mysqli_fetch_array($sql_lay_giohang)){ 
			$sanpham_id = $row_thanhtoan['id_sach'];
			$giohang_id = $row_thanhtoan['id_giohang'];
			$soluong = $row_thanhtoan['soluong'];
			$sql_cthd = mysqli_query($con,"INSERT INTO cthd(id_hoadon,id_sach,soluong) values ('$id_hd','$sanpham_id','$soluong')");
			$sql_delete_thanhtoan = mysqli_query($con,"DELETE FROM giohang WHERE id_giohang='$giohang_id'");
		}
		header('location: dat-hang-momo-thanh-cong&id-'.$id_hd.'');
    }
?>
